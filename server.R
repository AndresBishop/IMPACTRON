...
############################
  ...
  output$scatter <- renderPlot({
    ...
    plot(data(), xaxt="n", type="p", pch=ifelse(datavar()==1,11,21), bg=ifelse(datavar()==4,"blue",ifelse(datavar()==3,"orange","red")), ylab=input$var1, xlab="Time", ylim=c(-3, 3))
      ...
      plot(data(), xaxt="n", type="p", pch=ifelse(datavar()==1,11,21), bg=ifelse(datavar()==4,"blue",ifelse(datavar()==3,"orange","red")), ylab=input$var1, xlab="Time", ylim=c(-45, 45))
    }
    ...
    for(i in 1:longit){
      ...
      return(d)})
      segments(inicio,mean(dataotro()),elementtable,mean(dataotro()), col = "red", lwd = 4, lty=3)
...
############################
  ...
  output$MapPlot1 <- renderLeaflet({
    ...
    leaflet(data = df) %>% addTiles(urlTemplate = tcu_map) %>%
      #addAwesomeMarkers(~lng, ~lat, label=~as.character(station))
      #addMarkers(~lng, ~lat,  label = ~as.character(station), layerID=~station clusterOptions = markerClusterOptions())%>%
  ...
  output$plot=renderPlotly({
    ...
    data<-reactive({d<-#subset(spain[c(paste("qc", input$var1, sep = ""), input$var1)], spain$code == input$stationscatter)
      #subset(d[input$var1], d$paste("qc", input$var1, sep = "") != 1)
      subset(spain[,c(input$var1)],spain$code == my_place)
    #subset(d[,c(input$var1)],spain$code == input$stationscatter)
    })
    plot(data()) 
